...
def signature_auth(realm, key_file, debug=False):
    ...
    timestamp.  If the request has no 'signature' header, or if it does but
    the signature and the timestamp do not match, i.e. if authentication fails,
...
def _verify_signature(signature, message, key_file):
    ...
    Given the signature, the corresponding message (time_millis) and
    ...
    try:
        # Secure PSS padding implementation
